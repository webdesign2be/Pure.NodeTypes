...
/*                                                                        *
 * This script belongs to the Pure.NodeTypes package                      *
 *                                                                        *
...
abstract class AbstractGridNodeTypePostprocessor implements NodeTypePostprocessorInterface {
	...
	protected function validateSettings() {
		...
		if (empty($this->settings['devices']) || !is_array($this->settings['devices']))
			throw new \TYPO3\Neos\Exception('Pure.NodeTypes.devices settings are missing. Please check Settings.yaml file!', 1396555463);

		if (empty($this->settings[static::$SETTINGS_SECTION]) || !is_array($this->settings[static::$SETTINGS_SECTION]))
			throw new \TYPO3\Neos\Exception('Pure.NodeTypes.gridSettings settings are missing. Please check Settings.yaml file!', 1396555464);
	}
}
